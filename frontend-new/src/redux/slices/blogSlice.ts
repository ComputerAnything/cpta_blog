...
import type { BlogState, BlogPost, User, Comment } from '../../types'
...
export const createPost = createAsyncThunk(
  'blog/createPost',
  async (postData: { title: string; content: string; topic_tags?: string }, { rejectWithValue }) => {
    const token = localStorage.getItem('token')
    try {
...
export const updatePost = createAsyncThunk(
  'blog/updatePost',
  async (postData: { id: number; title: string; content: string; topic_tags?: string }, { rejectWithValue }) => {
    const token = localStorage.getItem('token')
    try {
...
export const upvotePost = createAsyncThunk(
  ...
  async (postId: number, { rejectWithValue }) => {
    ...
    try {
      ...
      return { postId, ...response.data } as { postId: number; upvotes: number; downvotes: number }
    } catch (err: any) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createPost.rejected, (state, action) => {
        state.loading = false
        state.error = (action.payload as string) || action.error.message || 'Failed to create post'
      })
      // Update Post
