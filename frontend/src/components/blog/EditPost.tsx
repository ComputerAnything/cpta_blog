...
const EditPost: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isGuest) {
      setToastMessage('Guests cannot edit posts. Please create an account.')
      return
    }

    if (!user) {
      setToastMessage('Please login to edit a post.')
      return
    }
    ...
    try {
      ...
      setToastMessage(err.response?.data?.msg || 'Failed to update post')
      setSubmitting(false)
    }
  }
  ...
  useEffect(() => {
    if (!user && !isGuest) {
      ...
      setToastMessage('You are not authorized to edit this post.')
      setTimeout(() => navigate(`/posts/${postId}`), 3000)
    }
  }, [user, isGuest, post, navigate, postId])
  ...
  if (!user || isGuest || post.user_id !== user.id) {
    return null
  }
